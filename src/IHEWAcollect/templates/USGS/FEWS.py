...
def convert_data(args):
    ...
    part_file_name = 'et{dtime:%y%m%d}'.format(dtime=date)
    ...
    if current_conf['is_save_temp']:
        ...
        # print('temp {}'.format(path))
        # path = os.path.join(path, part_file_name+'.bil')
        # print('temp {}'.format(path))
...
def clean(path,log_file,part_file_name):
    ...
    if 'temporary' in path: 
        for root, dirs, files in os.walk(path):
            for folder in dirs:
                if(part_file_name in os.path.join(root, folder)):
