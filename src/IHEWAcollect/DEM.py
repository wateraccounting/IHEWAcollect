...
This module downloads DEM data from
...
::
    ...
    from wateraccounting.Collect import DEM
...
def Find_Document_names_15s_30s(latlim, lonlim, parameter, resolution):
    ...
    for continent in continents:
        ...
        if (extent[0] < lonlim[0] < extent[1]
            and
            extent[2] < latlim[0] < extent[3]) and \
                (extent[0] < lonlim[1] < extent[1]
                 and
                 extent[2] < latlim[1] < extent[3]) == True:
            if resolution == "15s":
